...
function handleSubscribe (client, packet, restore, done) {
  ...
  let isSubscribed = persistence.checkSubscribed(packet.subscriptions[0].topic, client);
  ...
  client.broker._parallel(
    ...
    // 如果是 true 做done ,如果是false 做completeSubscribe
    // TODO: 好像不用 是在connect得地方的樣子 斷線後重新連線也要開始重新轉發sub_outbox得地方
      isSubscribed ? completeSubscribe : completeFirstSubscribe)
}
...
function checkSubOutbox (sub, done) {
  const packet = this.packet
  const client = this.client
  ...
  // TODO: 加入檢查pub_outbox功能之後並處理是否成功建立suboutbox後

  if (typeof packet.properties !== 'undefined' && packet.properties.userProperties.e2e) {
    client.broker.persistence.createSubOutbox(client, packet, function isCreateSuccess (err) {
      done(err, sub)
    })
  } else {
    done(null, sub)
...
function completeFirstSubscribe (err) {
  ...
  // TODO: 真實要處理Outbox發送就是這裡才做判斷上面一次下面一次

  //    處理  retain 資訊
  const persistence = broker.persistence
  // const stream = persistence.createRetainedStreamCombi(topics)

  const stream = persistence.createSubOutboxStreamCombi(topics, client)
  // console.log("stream",stream)
  // console.log("stream.function",stream._callback.toString())
