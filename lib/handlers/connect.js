...
function Connack (arg) {
  ...
  this.reasonCode = 0
  this.properties = {}
  if (arg.responseInformation) {
    this.properties.responseInformation = arg.responseInformation
  }
}
...
function init (client, packet, done) {
  ...
  // 回執預設topic
  client.requestResponseInformation = packet.properties.requestResponseInformation || false
...
function doConnack (arg, done) {
  ...
  if (this.client.requestResponseInformation) {
    arg.responseInformation = 'endToEndResponses/' + client.id
  }
...
function emptyQueue (arg, done) {
  // TODO: 處理端線後原本publish或是pubrec沒有結束得訓封包
  //  我要加上sub_outbox的補發處理
  const client = this.client
  const persistence = client.broker.persistence
  // const outgoing = persistence.outgoingStream(client)
  const subOutboxOutgoing = persistence.outgoingSubOutboxStream(client)
  ...
  client.resume()
    ...
    pipeline(
        ...
        through(function clearQueue (data, enc, next) {
          ...
          persistence.updateSubOutbox(client, packet, emptyQueueFilter)
        }),
        done
    )
}
...
function emptyQueueFilter (err, client, packet) {
  ...
  if (client.clean || !authorized) {
    persistence.outgoingSubOutboxClearMessageId(client, packet, next)
  } else {
