...
function Client (broker, conn, req) {
  ...
  function nextBatch (err) {
    ...
    if (that._parsingBatch <= 0) {
      ...
      // console.log("client.conn.read(null)",client.conn.read(null))
      // 讀資料
      const buf = client.conn.read(null)
      if (buf) {
        // console.log("client._parser.parse",buf)
  ...
  // conn.on('readable', ()=>{console.log("conn.on('readable')我進來啦");})
  ...
  this.deliverQoS = function deliverQoS (_packet, cb) {
    // TODO: 加入回執功能 如果成功接回comp則 publish給 發送端的響應主題
    ...
    if (toForward) {
      setImmediate(() => {
        ...
        // 清理会话(CleanSession) 0為保留 1為要清理乾淨 不用儲存任何東西
        // packet.retain 1為retain 重新連接後要重送的
        if (that.clean || packet.retain) {
          writeQoS(null, that, packet)
        } else {
          //準備發送publish需要給封包加上messageID
          broker.persistence.updateSubOutbox(that, packet, writeQoS)
  ...
  // TODO: 測試用 連線後沒有傳遞connect封包來 就會報錯
