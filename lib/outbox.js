...
class Outbox {
  ...
  clone(_size) {
    let newOutbox = new Outbox(_size, this.property, this._outbox);
    newOutbox._outbox = newOutbox._outbox.slice(-_size);
    return newOutbox
  }
  ...
  //因為qos-2有好幾個步驟
  // 當一個步驟確定以後會將訊息更新像是原本保留的publish封包當broker收到pubrec後我們需要將封包更新成pubrel
  update (packet) {
    let temp
    for (let i = 0; i < this._outbox.length; i++) {
      temp = this._outbox[i]
  ...
  // 最後broker收到pubcomp以後將確定成功送出的訊息依照messageID刪除
  ...
  for (let i = 0; i < patterns.length; i++) {
    if(Object.keys(pubOutboxsByT).length !== 0){
      for (const box in subOutboxs) {
        if (box === patterns[i]) {
          //TODO:如果訂閱沒有的topic還沒做

          // console.log(patterns[i]);
          // console.log(pubOutboxsByT[patterns[i]]);
          subOutboxs[box] = pubOutboxsByT[patterns[i]].clone(subOutboxs[box].size);
...
MemoryPersistence.prototype.checkSubscribed = function (topic, client) {

  let subOutboxs = this._subOutboxsbyC[client.id] || []
  console.log(subOutboxs[topic]);
  if(subOutboxs[topic]===undefined){
    return 0
  }else{
    return 1
  }
...
MemoryPersistence.prototype.createSubOutboxStreamCombi = function (patterns, client) {
  ...
  // console.log(this._pubOutboxsByT);
  // console.log(pubOutboxsByT);
...
MemoryPersistence.prototype.createSubOutbox = function (client, packet, cb) {
  ...
  // console.log(this._subOutboxsbyC[id]==[]);
  // console.log(subOutboxs[packet.subscriptions[0].topic]);

  this._subOutboxsbyC[id] = subOutboxs

  if(subOutboxs[packet.subscriptions[0].topic]===undefined){
