...
MemoryPersistence.prototype.enqueueSubOutbox = function (subs, packet, cb) {
  for (let i = 0; i < subs.length; i++) {
    _subOutboxEnqueue.call(this, subs[i], packet)
  }
  process.nextTick(cb)
}

function _subOutboxEnqueue (sub, packet) {
  const id = sub.clientId
  const subOutbox = this._subOutboxs[id] || []

  this._subOutboxs[id] = subOutbox
  const p = new Packet(packet)
  subOutbox[sub].enqueue(p)
}
...
MemoryPersistence.prototype.updateSubOutbox = function (client, packet, cb) {
  ...
  for (const box in subOutbox) {
    result = subOutbox[box].update(packet)
    if (result) {
      console.log(result)
      cb(null, result)
    } else {
...
MemoryPersistence.prototype.dequeueSubOutbox = function (client, packet, cb) {
  ...
  let result
  for (const box in subOutbox) {
    result = subOutbox[box].take(packet)
    if (result) {
      console.log(result)
      cb(null, result)
    } else {
